...
public:
   ...
   void              UseMultipattern(bool value) {m_useMultiPattern=value;}
   void              PeriodBands(int value) {m_peiodBands=value;}
   void              BandsShift(int value) {m_bandsShift = value;}
...
//+------------------------------------------------------------------+
bool CSignalRSIBB::ValidationSettings(void)
  {
...
//+------------------------------------------------------------------+
bool CSignalRSIBB::InitRSI(CIndicators *indicators)
  {
//--- check pointer
   if(indicators==NULL)
      return(false);
...
//--- initialize object
   ...
   if(!m_bands.Create(Symbol(),PERIOD_CURRENT,m_peiodBands,0,2,m_rsi.Handle()))
     {
      printf(__FUNCTION__+": error initializing object");
      return(false);
     }
...
//+------------------------------------------------------------------+
int CSignalRSIBB::StateRSI(int ind)
  {
   int    res=0;
   double var;
//---
...
//+------------------------------------------------------------------+
bool CSignalRSIBB::ExtStateRSI(int ind)
  {
...
//+------------------------------------------------------------------+
bool CSignalRSIBB::CompareMaps(int map,int count,bool minimax,int start)
  {
   ...
   if(DiffRSI(idx)>0.0)
     {
      ...
      if(IS_PATTERN_USAGE(1) && DiffRSI(idx+1)<0.0 && RSI(idx+1)<m_bands.Lower(idx+m_bandsShift))//30.0)
         result=m_pattern_1;      // signal number 1
      //--- if the model 2, 3, 4 or 5 is used, perform the extended analysis of the oscillator state

      if(m_useMultiPattern)
        {
   ...
   if(DiffRSI(idx)<0.0)
     {
      ...
      if(IS_PATTERN_USAGE(1) && DiffRSI(idx+1)>0.0 && RSI(idx+1)>m_bands.Upper(idx+m_bandsShift))//70.0)
         result=m_pattern_1;      // signal number 1
